...
complexidade, entre os problemas combinatoriais mais difícies de serem
...
medida de dispersão dos grupos a distância dos pontos em relação ao centróide
...
O problema das P-Medianas é uma variação do problema das k-médias, pelo fato
de utilizar uma mediana (um dos pontos) como referência para medição
das similaridades dos pontos de cada um dos agrupamentos.
%Esse problema é
%utilizado quenso é desejado, por exemplo, intalar um depósito em uma das loja
%de uma cidade por exemplo, desejando min

Acrescentando a restrição de capacidade aos agrupamentos, temos o problemas da
P-Medianas Capacitado ou também tratado na literatura pelo nome de Problema de
Agrupamento Capacitado (CCP)


\section{CCP - Problema de Agrupamento Capacitado}
Como citado anteriormente, o CCP tem por objectivo agrupar os pontos de modo a minimizar a distância entre entre todos pontos de um agrupamento e sua mediana. Na Fig. \ref{fig:Exemplo-CCP} é apresentado um exemplo de uma possível solução do CCP dado um conjunto de pontos.
...
agrupamento. O problema pode ser representado pela seguinte formulação
matemática \cite{ahmadi2004density}:
...
entre os componentes de cada agrupamento. Para o $\Re^{2}$, utilizamos a
distância como
