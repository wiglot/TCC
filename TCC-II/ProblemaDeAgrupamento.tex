...
agrupamento: reunir dados por semelhança.
Agrupamento é o ato de reunir objetos(dados) com certas caracteristicas semelhantes em um mesmo grupo,
 minimizando a variação de tal característica dos objetos dentro dos grupos.
Esse é um problema muito encontrado quando desejamos sumarizar os dados para uma melhor análise global. Dentre a aplicação encontram-se:
Redução de cores de imagens, localização de facilidades, reconhecimento de padrões, alocação de tarefas a máquinas

Muitas vezes temos um volume muito grande de dados para analisar sendo uma tarefa complexa, se não até impossível, analisar um a um todos os dados.
Uma forma de sumarizar tais dados seria a redução dos dados para apenas os dados mais representativos, agrupando os demais dados em volta desses dados.
Muitos problemas podem ser reduzidos a um problema de agrupamento como por exemplo:
\begin{itemize}
 \item
\end{itemize}


Quando falamos em organizar ou analisar grandes volumes de dados de modo a procurar

Quanto existem dados que é desejado agrupar os mesmos por semelhança, por exemplo pixels de uma imagem agrupados por semelhança de cor
, desejamos que os grupos sejam os mais concissos quanto possível, evitando a generalização dos mesmos.
A escala do problema de agrupamento é dado pela expressão de distintas partições $p$ em $n$ pontos,$N(n,p)$,  definida por \cite{liu1968cl}:
\begin{equation}
 N(p,n) = \frac{1}{p!}\sum_{i=0}^{p}(-1)^{p-1}\binom{p}{i}i^{n}
\end{equation}
Ahmadi e Osmam apresentam em \cite{ahmadi2004density} um exemplo do resultado da computação dessa expressão para uma instância com 50 pontos, o resultado pode ser visto na imagem \ref{ahmadiCalc}.
\begin{figure}
 \begin{center}
...
\end{center}
\caption[Valores para N(n,p)]{\label{ahmadiCalc}Valores para N(n, p). No topo p = 1 e na base p = 50.}
\end{figure}

Como pode ser visto que mesmo para instâncias pequenas, o espaço de busca é imenso. || e o que mais?? ||
