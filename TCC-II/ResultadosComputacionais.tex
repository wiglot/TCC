...
Na tabela \ref{tab:valoresInstancias} apresenta os valores obtidos para os 5 algoritmos implementados. Foi colocado também os valores para n (número de indivíduos) e p (número de agrupamentos). Os valores apresentados são os mínimos para as heurísticas que utilizam aleatoriedade. Para melhor visualização os dados foram representados pelos gráficos \ref{fig:graphicSJC} e \ref{fig:graphicAE}. Logo após, na tabela \ref{tab:TemposInstancias}, apresentamos os tempos tomados para executar o processamento das soluções apresentadas como mínimas.
\begin{table}[htp]
\begin{center}
...
\begin{figure}[hp]
 ...
 \caption{Gráfico com os valores obtidos para as instâncias SJC}
...
\begin{figure}[hp]
 ...
 % GraphicSJC_Value.png: 740x573 pixel, 96dpi, 19.58x15.16 cm, bb=0 0 555 430
 \caption{Gráfico com os valores obtidos para as instâncias AE}
 \label{fig:graphicAE}
\end{figure}
...
A heurística Random Density foi a que obteve os menores valores na maioria das instâncias, seguida da Heurística Density, seguida das heurísticas HMeans e JMeans que apresentaram soluções semelhantes. a Heurística farthest apresentou resultados nada bons em comparação com as demais, mas os tempos de processamento são bastante baixos, sendo uma heurística 'barata' caso se deseje apenas trabalhar com a busca local, usando de técnicas mais pesadas como Simulated Anneling e Busca Tabu, como aplicado por Osman e Christofides fazem em \cite{osman1994capacitated}. Isso se deve em parte pela escolha gulosa levando em consideração apenas a distância dos pontos para a seleção dos pontos iniciais e por executar apenas uma vez a alocação dos pontos e escolha dos melhores centros.
