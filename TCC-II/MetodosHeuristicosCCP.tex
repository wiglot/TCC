...
Selecione P pontos como centros iniciais;
...
Selecione P partições dos pontos que serão os agrupamentos (C)

Encontre os pontos não ocupados (afastados do centro do agrupamento por uma tolerância)

Para cada ponto não ocupado, tente substituir um centro existente por ele. Mantenha o que apresentar maior redução na função objetivo

Se valor da função objetivo da solução nova (C')é menor que C, C=C'e volte ao passo 2. Caso contrario Pare.
\end{algorithm}
