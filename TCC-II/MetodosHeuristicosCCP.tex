...
\ENDFOR
\STATE $c \leftarrow Centroid( \frac{x}{np} , \frac{y}{np} )$ %\COMMENTS Construct the centroid
% \REPEAT
...
\hrule
Explicar Escolha dos melhores centros dos agrupamentos usando a distância do centróide.
\vfill
% \item [{CCCP}]~
%
% \begin{description}
...
Com essa metodologia, conseguimos reduzir o espaço de busca pela elimininação das soluções que não apresentariam reduções no valor da função objetivo. Uma rápida analise na complexidade $O(n \cdot Q \cdot K)$, onde Q = 30\% dos agrupamentos ($p\cdot 0.3$) e K é 50\% da média de pontos por agrupamento ficando uma complexidade muito próxima de $O(n)$ para cada iteração.
